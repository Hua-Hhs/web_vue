    ...
    <div class="table">
      ...
      <el-table :data="tableData" ref='table' style="width: auto">
        ...
        <el-table-column  label=" " width="100">            
          <template slot-scope="scope">      
            <el-button v-if="scope.row.stuid != 'null'" type="danger" @click="handleDelete(scope.row)" 
...
export default{
  data() {
    return {
      ...
      newtableData : [{stuid : '在此行添加选课新学生',name : '', classid : '', password : '', isEditing : false} ],
      ...
      uploadURL : this.GLOBAL.URL + 'CourseStudent/upload',
        fileList: [], // 上传的文件列表
    };
  },
  mounted() {
  ...
  methods: {    
    downloadExcel() {
      ...
      link.href = url;
      link.download = '课程学生.xlsx'; // 设置下载文件的文件名
    ...
    handleDelete(row){
      if(row.isDeleting){
        const data = {
        courseid : this.$globalVariable.courseID,
          stuid : row.stuid,          
        }
      this.$http.post(this.GLOBAL.URL + 'CourseStudent/DeleteCourseStudent', data).then(res => { 
                      const info = res.data
                      if(info.code != 20000) {
                          Message({
                              message: 'Error',
                        ...
                        Message({
                          ...
                          this.handleShowStudents();
                          // const index = this.tableData.findIndex(item => item.stuid === row.stuid);
                          // if (index !== -1) {
    ...
    handleAdd(row){        
      ...
      if(row.isEditing){
        ...
        this.$http.post(this.GLOBAL.URL + 'CourseStudent/InsertCourseStudent', data).then(res => { 
                      const info = res.data
                      console.log(info);
                      if(info.code == 20000) {
                          Message({
    ...
    upload() {      
      ...
      const data = {courseid : this.$globalVariable.courseID}
      // console.log(data)
      this.$http.post(this.GLOBAL.URL + 'CourseStudent/SetCurrentCourseid', data) .then(response => {
        const classid = response.data
        // console.log(classid)
        if(classid.code == 20000)
        this.$http.post(this.GLOBAL.URL + 'CourseStudent/upload', formData)
        .then(response => {
